...
/**
 * @brief Print build configuration
 */
void printBuildInfo(void)
{
    ...
    printf("Build: PARALLEL (with threading support)\n");
#ifdef _OPENMP
    printf("OpenMP: Available (version %d)\n", _OPENMP);
    printf("Max threads: %d\n", omp_get_max_threads());
#else
    printf("OpenMP: NOT available (will run sequentially)\n");
#endif
#else
    printf("Build: SEQUENTIAL\n");
#endif
...
int main(void)
{
    ...
    uint8_t halfWindowSize = 46;
    uint8_t polynomialOrder = 5;
    ...
    // Warmup run (initializes lookup tables)
    SAVGOL_FILTER_AUTO(rawData, dataSize, halfWindowSize, filteredData,
                       polynomialOrder, targetPoint, derivativeOrder, 4);
    ...
    {
        SAVGOL_FILTER_AUTO(rawData, dataSize, halfWindowSize, filteredData,
                           polynomialOrder, targetPoint, derivativeOrder, 16);
    }
    clock_t toc = clock();
    ...
    double throughput = (dataSize * iterations) / total_time;
